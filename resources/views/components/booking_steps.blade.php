<!--====== SERVICE PART START ======-->
<section class="service-section-two pt-20 pb-70">
    <div class="container">
        <div class="section-title text-center both-border mb-20">
            <h2>Simply book in 4 steps</h2>
            <p class="title-tag">Book Appointment</p>
        </div>
        <!-- Service Loop -->
        <div class="row service-loop-two">
            <div class="col-xl-3 col-lg-3 col-sm-6 wow fadeInUp" data-wow-delay=".3s">
                <div class="single-service-two text-center">
                    <div class="service-img mb-40 mx-5">
                        <img src="{{ asset('images/booking.webp') }}" alt="Icon">
                        <span>01</span>
                    </div>
                    <div class="service-content">
                        <p>Book your services online or over the phone</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-3 col-sm-6 wow fadeInUp" data-wow-delay=".4s">
                <div class="single-service-two text-center">
                    <div class="service-img mb-40 mx-5">
                        <img src="{{ asset('images/portfolio.webp') }}" alt="Icon">
                        <span>02</span>
                    </div>
                    <div class="service-content">
                        <p>Cleanovative matches you with your trusted professional</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-3 mx-auto col-sm-6 wow fadeInUp" data-wow-delay=".5s">
                <div class="single-service-two text-center">
                    <div class="service-img mb-40 mx-5">
                        <img src="{{ asset('images/login.webp') }}" alt="Icon">
                        <span>03</span>
                    </div>
                    <div class="service-content">
                        <p>Manage your bookings online through your own login portal</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-3 mx-auto col-sm-6 wow fadeInUp" data-wow-delay=".5s">
                <div class="single-service-two text-center">
                    <div class="service-img mb-40 mx-5">
                        <img src="{{ asset('images/coffee-break.webp') }}" alt="Icon">
                        <span>04</span>
                    </div>
                    <div class="service-content">
                        <p>Relax and enjoy your clean space</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!--====== SERVICE PART END ======-->
